resource "aws_db_instance" "default" {
  ...
  db_name                 = "mydb"
  identifier              = "rds-test2222222222222"
  engine                  = "mysql"
  engine_version          = "8.0"
  instance_class          = "db.t3.micro"
  username                = "admin"
  password                = "Cloud123"
  db_subnet_group_name    = aws_db_subnet_group.sub-grp1.name
...
resource "aws_iam_role" "rds_monitoring" {
  name = "ajay222222222-rds-monitoring-role"
...
# Subnet group for primary region

resource "aws_db_subnet_group" "sub-grp1" {
    provider   = aws.primary
  name       = "mycutsubnetssssssssssssssssssss"
